      ...
      <div className="max-w-7xl mx-auto">
        ...
        {tab === 'productos' && (
          ...
          <div className="bg-white rounded-xl border shadow-sm p-5 mb-10">
            ...
            <h2 className="text-lg font-semibold mb-4">Análisis de productos (MacBook)</h2>

            {(() => {

              const groups = data?.productGroups || [];

              const gamas = Array.from(new Set(groups.map(g=>g.gama).filter(Boolean)));

              const procs = Array.from(new Set(groups.map(g=>g.proc).filter(Boolean)));

              const pantallas = Array.from(new Set(groups.map(g=>g.pantalla).filter(Boolean)));

              const gSel = (typeof window !== 'undefined' && window.__anal_prod_g) || '';

              const pSel = (typeof window !== 'undefined' && window.__anal_prod_p) || '';

              const tSel = (typeof window !== 'undefined' && window.__anal_prod_t) || '';

              const applyFilter = (g) => (!gSel || g.gama===gSel) && (!pSel || g.proc===pSel) && (!tSel || g.pantalla===tSel);

              const filtered = groups.filter(applyFilter);
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-3 mb-4">
                    ...
                    <label className="text-sm">Gama
                      ...
                      <select className="ml-2 border rounded px-2 py-1 text-sm" defaultValue={gSel} onChange={(e)=>{ if (typeof window!== 'undefined') window.__anal_prod_g = e.target.value; }}>
                    ...
                    <label className="text-sm">Procesador
                      ...
                      <select className="ml-2 border rounded px-2 py-1 text-sm" defaultValue={pSel} onChange={(e)=>{ if (typeof window!== 'undefined') window.__anal_prod_p = e.target.value; }}>
                    ...
                    <label className="text-sm">Pantalla
                      ...
                      <select className="ml-2 border rounded px-2 py-1 text-sm" defaultValue={tSel} onChange={(e)=>{ if (typeof window!== 'undefined') window.__anal_prod_t = e.target.value; }}>
