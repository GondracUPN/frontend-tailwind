...
export default function ModalEditarEfectivo({ onClose, onSaved, current }) {
  ...
  const usdActual = Number(current?.efectivoUsd ?? 0);
  ...
  const submit = async (e) => {
    ...
    if (!isFinite(nPen) || nPen < 0) return setErr('Efectivo PEN invǭlido.');

    const token = localStorage.getItem('token');
    if (!token) return setErr('No hay sesi��n.');
    ...
    try {
      const res = await fetch(`${API_URL}/wallet`, {
        ...
        body: JSON.stringify({ efectivoPen: nPen, efectivoUsd: usdActual }),
      });
