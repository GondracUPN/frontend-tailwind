...
// Tipo de cambio fijo para Crédito
...
export default function ModalGastoCredito({ onClose, onSaved, userId, mode = 'create', initial = null }) {
  ...
  const onSubmit = async (e) => {
    ...
    if (isCompra) {
      ...
      // Nota: no enviamos campos no soportados por el backend aún
      // if (isCuotas) body.cuotas = Number(cuotas);
      // if (isRecurrente) body.recurrente = true;
    }
    ...
    >
      ...
      >
        ...
        <form className="grid gap-3" onSubmit={onSubmit}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <label className="text-sm">
              ...
              <div className="mt-1 text-xs text-gray-500 min-h-[1rem]">
                {moneda === 'USD' ? (
                  <span>≈ S/ {Number((Number(monto || 0) * TC_CREDITO).toFixed(2))} (TC {TC_CREDITO})</span>
                ) : null}
              </div>
            </label>
          ...
          {/* Deuda en cuotas: selector de cuotas */}
          {isCuotas && (
            <label className="text-sm">
          ...
          {/* Gastos recurrentes: aviso */}
          {isRecurrente && (
            <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-3 py-2">
              Este gasto se registrará cada mes en la misma fecha automáticamente (cuando el backend esté disponible).
            </div>
          )}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {saving ? (mode==='edit' ? 'Actualizando…' : 'Guardando…') : (mode==='edit' ? 'Actualizar' : 'Guardar')}
            </button>
          </div>
        </form>
      </div>
