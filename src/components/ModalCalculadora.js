...
export default function ModalCalculadora({ producto, onClose }) {
  ...
  const { pvMin, pvMed } = useMemo(() => {
    const pvMinRounded = roundUp10(costoTotalBase * 1.2); // +20%
    const pvMedRounded = roundUp10(costoTotalBase * 1.4); // +40%
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="text-sm text-gray-700 mt-3 space-y-1">
            ...
            <div className="flex justify-between">
              ...
              <strong>{fmtSoles(costoUsado)}</strong>
            </div>
            <div className="flex justify-between pt-1">
              <span>Precio aplicado (x10 arriba):</span>
            ...
            <div className="flex justify-between">
              ...
              <strong>{isNaN(customOut.pct) ? '-' : `${customOut.pct.toFixed(2)} %`}</strong>
            </div>
          </div>
        </div>
      </div>
